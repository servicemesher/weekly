...
你已经浏览了Istio Mixer Adapter的指南，现在想要发布自己的Adapter？这篇文章将教会你在生产环境中创建Adapter的过程。

![Istio](https://ws4.sinaimg.cn/large/006tNbRwly1fw3hlzj8krj304o06m3ya.jpg)

## 介绍

根据你对[Go](https://golang.org/)、[Protobufs](https://developers.google.com/protocol-buffers/)、[gRPC](https://grpc.io/)、[Istio](https://istio.io/)、[Docker](https://www.docker.com/)和[Kubernetes](https://kubernetes.io/)的了解，你可能会发现发布一个Istio Mixer Adapter的过程并不容易。本文假设你对这些技术有一些经验，并且根据Istio的Wiki已经能够完成至少一个演练。

依据本文的目的，我将讨论构建一个使用[Metrics](https://preary.istio.io/docs/reference/config/policy -telemetry/templates/metric/)的Istio Mixer Adapter。这是我们将要看到的一个简要步骤：

1. Istio Mixer - 适配器接口架构
2. 创建一个简单的Mixer Adapter
3. 将适配器发布到Docker Hub
4. 为适配器编写Kubernetes Config
5. 在Istio上部署和测试适配器

再次强调，我将尽最大努力在这篇文章里呈现所有重要的细节，让你的新适配器活起来。

## Istio Mixer - 适配器接口架构

让我们首先看看适配器如何与Istio Mixer接合。Kubernetes在一定程度上抽象了接口；然而，理解这一点对我们来说很重要，至少要有一点细节。
...
- **Mixer**是Istio组件，它从Proxy接收指标(和其他)数据，并转发给其他组件(在本例中是适配器)
- **Adapter**是我们正在构建的应用程序，通过gRPC消费来自Mixer的度量数据
- **Operator**扮演负责配置和部署的角色，在本例中是Istio和适配器

这里需要注意重要的一点是，每个组件都作为独立的进程运行，并且可能分布在网络上。此外，mixer还与适配器建立了一个gRPC通道，以便为其提供用户的配置和度量数据。

## 创建一个简单的Mixer Adapter

简便起见,我依据Istio [Mixer Out of Tree Adapter Walkthrough](https://github.com/istio/istio/wiki/Mixer-Out-of-Tree-Adapter-Walkthrough)这篇文章建立一个简单的mixer adapter。下面是一个概要，列出了创建消费度量数据的简单Istio mixer adapter要遵循的步骤。

1. 创建Adapter的配置文件`config.proto`
2. 创建`mygrpcadapter.go`文件，实现`HandleMetric(context.Context, *metric.HandleMetricRequest) (*v1beta11.ReportResult, error)`这个gRPC API的调用方法
3. 通过 `go generate ./...`生成配置文件
4. 创建`main.go`文件作为gRPC服务端并监听API调用
...
在本地安装和测试了`myootadapter`项目后，就可以构建并发布adapter到[Docker Hub](https://hub.docker.com/)这样的代码库了。在继续前请执行以下步骤。
...
`CMD [ "8000" ]` 告诉Docker，将`8000`作为参数，传递给定义在行`ENTRYPOINT [ "/bin/mygrpcadapter" ]`的接口。 因为我们在这里把gRPC的监听端口改为了 `8000`，必须更新文件`sample_operator_config.yaml` 使其一致。我们把原来的配置 `address: "{ADDRESS}"` 用 `address: mygrpcadapter:8000`替换。
...
上面的配置定义了一个简单的服务，只有一个从`dockerhub-username/mygrpcadapter:latest`的映像创建的副本。该服务可以被命名为`mygrpcadapter`，并可以通过端口`8000`访问。这就是如何让配置在`sample_operator_config.yaml` 的 `address: "mygrpcadapter:8000"` 指向这个特殊的deployment方法。
...
这告诉Kubernetes调度器，不要自动注入Istio的sidecar。这样做是因为我们并不需要在Adapter前面有一个Proxy。另外，第二个标注将这个pod标记为系统的critical级别。

我们还创建了一个名为`transient-storage`的临时volume，它用于存储适配器输出，例如`out.txt`文件。配置中如下的代码片段使我们能够做到这一点：
...
Istio提供了一种标准机制来管理和观测云环境下的微服务。Mixer让开发人员能够轻松地将Istio扩展到自定义平台。我希望这篇指南让你对Istio Mixer - Adapter接口有一个初步了解，以及如何自己构建一个生产就绪的Adapter!

------

去发布你自己的Istio Mixer Adapter! 别介意用[Wavefront by VMware Adapter for Istio](https://github.com/vmware/wavefront-adapter-for-istio) 做参考。

如果你希望在[Istio Adapters](https://github.com/istio/istio/wiki/publish-adapters-templats-istio.io)页面上发布您的Adapter，请参考这个[Wiki](https://istio.io/docs/reference/reference/config/policy- telemetry/adapters/)。
